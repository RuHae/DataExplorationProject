...
def baseline_model(num_classes, loss="binary_crossentropy", optimizer = SGD(lr=0.0005, decay=1e-6, momentum=0.9, nesterov=True), metrics=['accuracy']):
    
    # create model architecture
    model = Sequential()
    
    model.add(Conv2D(32, kernel_size=(3,3), input_shape=(3, 50, 50), activation='relu', padding="same"))
...
#####################
    ...
    #def baseline_model(num_classes, loss="binary_crossentropy", optimizer = Adam(), metrics=['accuracy']):
...
#################################################################################################################################
    ...
    ## model.add(Conv2D(128, kernel_size=(3,3), input_shape=(3, 50, 50), activation='relu', padding="same"))
...
def run_training(model_json_file="model.json", model_weights_file="model_weights.h5"):
    ...
    num_classes, X_total, y_total, split_index = data_preparation()
    ...
    # Fit the model
    history = model.fit(X_total[:split_index], y_total[:split_index], validation_data=(X_total[split_index:], y_total[split_index:]), epochs=65, batch_size=64, callbacks=callbacks_list, verbose = 1)
    ...
    return scores(model, X_total[split_index:], y_total[split_index:])